...
const AIHeadhunters = ({ activeTab }: AIHeadhuntersProps) => {
  return (
    <div className="container mx-auto px-4 sm:px-6 pt-0 pb-8 sm:pb-12">
      <div className="bg-white rounded-2xl sm:rounded-3xl transition-all duration-500 overflow-hidden shadow-sm border border-black/[0.03]">
        <div className="max-w-4xl mx-auto px-6 sm:px-12 md:px-16 py-16 sm:py-24">
          <div className="text-center space-y-6 sm:space-y-8">
            {/* Badge */}
            <BrandBadge label={activeTab === 'for talent' ? 'AI Headhunter' : 'AI Super Connector'} />

            {/* Name */}
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-gray-900 tracking-tight">
              {activeTab === 'for talent' ? 'Tess' : 'Tom'}
            </h2>
            ...
            <h3 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 max-w-3xl mx-auto leading-tight mt-6 sm:mt-8">
              {activeTab === 'for talent'
                ? 'I help you find companies that fit, fast.'
                : 'I connect you to the best talent'
              }
            </h3>
            ...
            <p className="text-gray-500 text-lg sm:text-xl md:text-2xl leading-relaxed max-w-2xl mx-auto">
              {activeTab === 'for talent'
                ? 'I\'ll find you opportunities so you can focus on what you do best. I\'ll connect you to great companies Tom works with. ✨'
                : 'I listen to your needs and connect you with the best upcoming talent. Together, we can increase your brand awareness and reach. I\'ll introduce you to the right people Tess is working with. 🚀'
              }
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mt-8 sm:mt-16">
              <a 
                href={activeTab === 'for talent' ? 'https://tally.so/r/w77VJ6' : 'https://tally.so/r/mDRJXX'}
                target="_blank"
              ...
              <Link 
                to={activeTab === 'for talent' ? '/companies' : '/talent'}
              ...
              >
                ...
                <span className="relative z-10 text-white/90">
                  {activeTab === 'for talent' 
                    ? 'Not talent? Switch to company mode →'
                    : 'Not a company? Switch to talent mode →'
                  }
                </span>
              </Link>
            </div>
